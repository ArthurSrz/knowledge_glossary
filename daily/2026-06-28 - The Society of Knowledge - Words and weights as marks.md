...
```mermaid
graph LR
    LEAD["…just marks, until…"]:::ghost --- W["a word / a weight"]
    W -->|"alone"| N["∅"]
    W -->|"+ context"| M["meaning"]
    M --- TAIL["…meaning is on loan."]:::ghost
    classDef ghost fill:none,stroke:none,color:#333,font-size:20px,font-style:italic,font-weight:bold
    style W fill:#fff4c2,stroke:#333
    style N fill:#fafafa,stroke:#ccc
...
```mermaid
graph LR
    LEAD["…not pointing, but pushing…"]:::ghost --- OLD["❌ denotes meaning"]
    OLD --> NEW["✅ triggers → triggers → triggers"]
    NEW --- TAIL["…a cascade."]:::ghost
    classDef ghost fill:none,stroke:none,color:#333,font-size:20px,font-style:italic,font-weight:bold
...
```mermaid
graph TB
    LEAD["…what you read is only the tip…"]:::ghost --- VIS["🔍 words / tokens"]
    HID["🌫️ hidden ocean of weights"] -->|"surfaces as"| VIS
    HID --- TAIL["…the rest stays under."]:::ghost
    classDef ghost fill:none,stroke:none,color:#333,font-size:20px,font-style:italic,font-weight:bold
    style VIS fill:#fff4c2,stroke:#333
    style HID fill:#eceff1,stroke:#607d8b
```
...
```mermaid
graph LR
    LEAD["…between two unknowns…"]:::ghost --- Q1
    Q1((❓)) -.-> I["« I want to read »"]
    I -.-> C["« click »"]
    C -.-> Q2((❓))
    Q2 --- TAIL["…we don't know either end."]:::ghost
    classDef ghost fill:none,stroke:none,color:#333,font-size:20px,font-style:italic,font-weight:bold
...
```mermaid
graph LR
    LEAD["…the trick reverses…"]:::ghost --- F["📦 full → rigid"]
    F --- E["📭 empty → versatile"]
    E --- TAIL["…emptiness is the gift."]:::ghost
    classDef ghost fill:none,stroke:none,color:#333,font-size:20px,font-style:italic,font-weight:bold
