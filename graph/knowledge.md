---
has cause:
  - "[[graph/learning]]"
  - "[[graph/revelation]]"
  - "[[graph/qualia]]"
partially coincident with:
  - "[[graph/experience]]"
  - "[[graph/understanding]]"
used by: "[[graph/knowledge management]]"
described by source:
  - "[[graph/The Catholic Encyclopedia]]"
  - "[[graph/Gujin Tushu Jicheng]]"
  - "[[graph/Otto's encyclopedia]]"
different from:
  - "[[graph/Knowledge]]"
  - "[[graph/Kila]]"
  - "[[graph/Wissen]]"
contributing factor of:
  - "[[graph/competence]]"
  - "[[graph/understanding]]"
opposite of: "[[graph/ignorance]]"
studied by:
  - "[[graph/sociology of knowledge]]"
  - "[[graph/theory of knowledge]]"
  - "[[graph/philosophy]]"
  - "[[graph/epistemology]]"
said to be the same as: "[[graph/certainty]]"
has characteristic:
  - "[[graph/knowledge environment]]"
  - "[[graph/limits of knowledge]]"
  - "[[graph/knowledge type]]"
Wikimedia outline: "[[graph/outline of knowledge]]"
history of topic: "[[graph/history of knowledge]]"
union of: "[[graph/list of values as qualifiers]]"
instance of: "[[graph/philosophical concept]]"
subclass of: "[[graph/memory]]"
permanent duplicated item: "[[graph/Q117833695]]"
OmegaWiki Defined Meaning: "8484"
Dewey Decimal Classification: "001"
IMDb keyword: knowledge
PhilPapers topic: knowledge
Iconclass notation: 52A5
Commons category: Knowledge
pronunciation audio: http://commons.wikimedia.org/wiki/Special:FilePath/LL-Q1571%20%28mar%29-Neelima64-%E0%A4%9C%E0%A5%8D%E0%A4%9E%E0%A4%BE%E0%A4%A8.wav
image: http://commons.wikimedia.org/wiki/Special:FilePath/Tetradrachm%20Athens%20480-420BC%20MBA%20Lyon.jpg
Stack Exchange tag: https://philosophy.stackexchange.com/tags/knowledge
UMLS CUI: C0376553
MeSH tree code: K01.468
schematic: http://commons.wikimedia.org/wiki/Special:FilePath/Classical%20definition%20of%20Kno%20uk.svg
wikidata entity id: Q9081
broader: "[[Process]]"
inferred:skos:ancestor:
  - "[[Infrastructure]]"
  - "[[Memory]]"
  - "[[Process]]"
inferred:skos:broader: "[[Memory]]"
inferred:skos:narrower:
  - "[[Knowledge base]]"
  - "[[La Société de Connaissance]]"
  - "[[field of study (education)]]"
  - "[[knowledge worker]]"
---
